...
public class Example {
    public static void main(String[] args) throws Throwable {
        System.loadLibrary("isdoc");

        var linker = Linker.nativeLinker();
        var lookup = SymbolLookup.loaderLookup();

        var parseFunc = linker.downcallHandle(
            lookup.find("isdoc_parse").orElseThrow(),
            FunctionDescriptor.of(ValueLayout.ADDRESS, ValueLayout.ADDRESS)
        );

        var validateFunc = linker.downcallHandle(
            lookup.find("isdoc_validate").orElseThrow(),
            FunctionDescriptor.of(ValueLayout.ADDRESS, ValueLayout.ADDRESS)
        );

        var freeFunc = linker.downcallHandle(
            lookup.find("isdoc_free").orElseThrow(),
  ...
  <LegalMonetaryTotal>
    ...
    <PayableAmount>1210.00</PayableAmount>
        ...
        try (var arena = Arena.ofConfined()) {
            var xmlPtr = arena.allocateUtf8String(xml);

            var resultPtr = (MemorySegment) parseFunc.invoke(xmlPtr);
            var parseResult = resultPtr.reinterpret(Integer.MAX_VALUE).getUtf8String(0L);
            System.out.println("Parsed Invoice ID: FV-2025-001");
            freeFunc.invoke(resultPtr);

            var validateResultPtr = (MemorySegment) validateFunc.invoke(xmlPtr);
            var validateResult = validateResultPtr.reinterpret(Integer.MAX_VALUE).getUtf8String(0L);
            System.out.println("Valid: " + validateResult.contains("\"valid\":true"));
            freeFunc.invoke(validateResultPtr);
        }
    }
}
