...
#if os(Linux)
let libName = "libisdoc.so"
#elseif os(macOS)
let libName = "libisdoc.dylib"
#else
let libName = "libisdoc.dll"
#endif
...
typealias ParseFunc = @convention(c) (UnsafePointer<CChar>?) -> UnsafePointer<CChar>?
typealias ValidateFunc = @convention(c) (UnsafePointer<CChar>?) -> UnsafePointer<CChar>?
typealias FreeFunc = @convention(c) (UnsafeMutablePointer<CChar>?) -> Void

let parse = unsafeBitCast(dlsym(lib, "isdoc_parse"), to: ParseFunc.self)
let validate = unsafeBitCast(dlsym(lib, "isdoc_validate"), to: ValidateFunc.self)
let free = unsafeBitCast(dlsym(lib, "isdoc_free"), to: FreeFunc.self)
...
let resultPtr = parse(xml)
let result = String(cString: resultPtr!)
free(UnsafeMutablePointer(mutating: resultPtr))

if let data = try? JSONSerialization.jsonObject(with: result.data(using: .utf8)!) as? [String: Any] {
    print("Parsed Invoice ID: \(data["ID"] ?? "N/A")")
}

let validatePtr = validate(xml)
let validateResult = String(cString: validatePtr!)
free(UnsafeMutablePointer(mutating: validatePtr))

if let validation = try? JSONSerialization.jsonObject(with: validateResult.data(using: .utf8)!) as? [String: Any] {
    print("Valid: \(validation["valid"] ?? false)")
}
